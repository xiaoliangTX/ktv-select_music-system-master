<template>
  <div class="managemusic">
    ...
    <Row style="padding: 20px 60px 20px 40px">
      ...
      <Col :span="4">
        <Button type="success" style="margin-left: 5px" @click="toListen = true"
          >试听歌曲</Button
        >
      </Col>
    ...
    <div style="width: 100%; background-color: #f40">
      <el-table :data="allSongs" class="song-table" style="width: 100%" border>
        ...
        <el-table-column prop="_id" label="歌曲 ID" align="center" width="230">
        </el-table-column>
        <el-table-column
          label="歌曲名"
          prop="songName"
        ...
        <el-table-column prop="artist" label="歌手" align="center" width="150">
        </el-table-column>
        <el-table-column prop="src" align="center" width="210" label="歌曲地址">
        </el-table-column>
        <el-table-column align="center" width="100" label="歌曲封面">
          <template slot-scope="scope">
            <Avatar
              :src="
                'http://localhost:8633/api/music/poster?img=' + scope.row.poster
              "
              size="large"
            />
          </template>
        </el-table-column>
        <el-table-column align="center" prop="language" width="90" label="语种">
        </el-table-column>
        <el-table-column align="center" prop="style" width="90" label="风格">
        </el-table-column>
        <el-table-column
          align="center"
          prop="playcount"
        ...
        >
          <template slot-scope="scope">
            <el-button
              ...
              :disabled="scope.row.isLike == true"
              title="收藏"
            ></el-button>
            <el-button
              type="primary"
        ...
        >
          ...
          <el-form-item prop="src" label="歌曲地址:">
            <Upload
              ...
              :format="['mp3']"
            >
              <Button icon="ios-cloud-upload-outline" @click.native="clearFiles"
                >上传歌曲</Button
              >
            </Upload>
          </el-form-item>
          ...
          <el-form-item prop="language" label="语种:">
            <Select v-model="addForm.language" clearable style="width: 200px">
              <Option v-for="item in Languages" :value="item" :key="item">{{
                item
              }}</Option>
            </Select>
          </el-form-item>
          <el-form-item prop="style" label="风格:">
            <Select v-model="addForm.style" clearable style="width: 200px">
              <Option v-for="item in SongStyle" :value="item" :key="item">{{
                item
              }}</Option>
            </Select>
          </el-form-item>
    ...
    >
      ...
      <div style="text-align: left; font-size: 14px">
        <Row>
          ...
          <Col span="6">
            <Button type="primary" size="large" @click="checkPwd()"
              >验证</Button
            >
          </Col>
        </Row>
        ...
        <div style="width: 100%">
          <el-table :data="allSongs" height="300" style="width: 100%">
            ...
            <el-table-column align="center" width="200" label="试听">
              <template slot-scope="scope">
                <span class="toListen-link" @click="listenMusic(scope.row)"
                  >播放</span
                >
              </template>
            </el-table-column>
...
<script>
import customLoading from "@/plugins/customLoading";
import $ from "jquery";
export default {
  name: "managemusic",
  data() {
  ...
  methods: {
    ...
    checkPwd() {
      if (this.inputPassword.trim().length) {
        ...
        customLoading.start("正在验证密码,请稍候...");
        setTimeout(() => {
          this.$axios
            ...
            .then((res) => {
              if (res.data == true) {
                if (this.flag == 1) {
                  console.log(111);
                  // 添加歌曲
                  customLoading.start("密码正确, 正在添加歌曲");
                  this.$axios
                    .post(
                  ...
                  // 编辑歌曲
                  customLoading.start("密码正确, 正在编辑歌曲歌曲");
                  this.$axios
                    .post(
                  ...
                  // 删除歌曲
                  customLoading.start("密码正确, 正在删除歌曲..");
                  this.$axios
                    .post(
    ...
    likeSongs(song) {
      console.log(song);
      this.$confirm(
        `要添加 ${song.songName} (${song.artist}) 歌曲到KTV推荐歌曲里吗? `,
        "操作提示",
        {
    ...
    onDeleteSong(song) {
      this.$confirm(
        `确定删除 ${song.songName} (${song.artist}) 歌曲吗?不可撤销,是否继续? `,
        "风险提示",
        {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
